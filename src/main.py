...
class ESOINN:
    ...
    def find_winners(self,inputVec):
        ...
        for j in range(2,self.neurons.shape[0]):
            ...
            if dist < min_dist2-0.0001:
                if dist < min_dist1-0.0001:
                    winners[1] = winners[0]
                    winners[0] = j
                    min_dist2 = min_dist1
                    min_dist1 = dist
    ...
    def merge_condition_algorithm1(self,A,B):
        ...
        # Если подклассы разные и есть между ними ребро
        if self.neurons[A].mark != self.neurons[B].mark:
            if self.adjacency_matrix[A][B].rib_exist == 1:
                ...
                if min(self.neurons[A].density,self.neurons[B].density) > \
                    param_of_A * max_dens_cluster_of_A or \
                    min(self.neurons[A].density,self.neurons[B].density) > \
                    param_of_B * max_dens_cluster_of_B:
    ...
    def mark_neurons(self):
        ...
        # Локальная область нейрона А - все нейроны, соединенные с нейроном Аф
            ...
            if max_dens_index == i:
                ...
                if len(list_of_apexes) == 0:
                    ...
                    for j in range(len(list_of_apexes)):
                        if self.neurons[i].density > list_of_apexes[j]:
            ...
            for j in range(i+1, self.adjacency_matrix.shape[1]):
                if self.adjacency_matrix[i][j].rib_exist == 1:
                    if self.neurons[i].mark != self.neurons[j].mark:
                        ...
                        # Разъединение узлов... Кстати говоря, тут и косяк вроде
                        else:
                            self.adjacency_matrix[i][j].rib_exist = 0
                            self.adjacency_matrix[j][i].rib_exist = 0
    ...
    def merge_subclasses(self,A,B):
        ...
        for i in range(self.neurons.shape[0]):
            if self.neurons[i].mark == temp_mark:
                self.neurons[i].mark = self.neurons[A].mark
...
ExampleStart = ESOINN(100, 0.001, 1, 200)
