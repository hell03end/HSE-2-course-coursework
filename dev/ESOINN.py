...
class _ESOINNNode:
    ...
    def __repr__(self) -> str:
        return f"{str(self.features):^30} | {self.subclass_id:10} | " \
               f"{float(self.density):13.10} | {float(self.points):13.10} | " \
               f"{float(self.__win_period):13.10} | {self.accumulate_signals}"
    ...
    def update_win_period(self, period: int) -> None:
    ...
    def update_density(self, coeff: float=None) -> None:
        if coeff:
            ...
            self.__density = self.__total_points/self.__win_period
